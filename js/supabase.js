...
// FIX: We use "window.supabase" instead of "const supabase" 
// so the HTML files can actually see the connection!
...
async function checkAdmin() {
  ...
  const { data } = await window.supabase.from('users').select('role').eq('id', user.id).single();
